...
async function fetchBlog(): Promise<void> {
  ...
  try {
    ...
    blog.value = result
    document.title = `${result.title} | Faramita BBS`
    await nextTick()
    await renderPreview()
  } catch (error) {
...
function downloadMarkdown(): void {
  ...
  link.href = url
  link.download = `${blog.value.title || 'faramita-blog'}.md`
  link.click()
...
function getPrintableHtml(): string {
  ...
  const title = escapeHtml(blog.value.title || 'Faramita Blog')
      ...
      <div v-else-if="blog" class="detail-shell">
        ...
        <main class="article-card">
          <header class="article-header">
            <div class="article-kicker">Faramita BBS</div>
            <h1>{{ blog.title }}</h1>
